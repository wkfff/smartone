...
<head>
	...
	$(function (){
		...
		$shiftGroupList = getShiftGroupList();
		f_initGrid();
	});
	//扩展一个 多行文本框 的编辑器
	...
	function getShiftGroupList(){
		//请求服务器
		var groupSelectList ;
		$.ajax({
			type: 'post',
			url: '/att/attendanceSettings/getJustShiftInfoList', 
			dataType: 'json',
			async: false,
			success: function (list){
				groupSelectList = list ;
			}
		});
	...
	function f_initGrid(){
		$grid = $("#shiftGroupInfo").ligerGrid({
			columns: [
			{ display: '<spring:message code="hrms.primaryKey"/>', name: 'GROUP_NO',width:0,hide:1},
			{ display: '<spring:message code="hrms.workingShift.code"/>', name: 'GROUP_ID',width:0,hide:1},
			{ display: '', name: 'SHIFT_ID',width:0,hide:1},
			{ display: '<spring:message code="hrms.chineseName"/>', name: 'GROUP_NAME',width:120, editor: { type: 'text' }},
			{ display: '<spring:message code="hrms.englishName"/>', name: 'GROUP_EN_NAME',width:120, editor: { type: 'text' }},
			{ display: '<spring:message code="hrms.workingShift.name"/>', name: 'SHIFT_NO', align: 'center',width: 100,isSort: false,
				editor: { 
					type: 'select', data: $shiftGroupList, dataValueField: 'SHIFT_NO',dataDisplayField: 'SHIFT_NAME', 
					displayColumnName: 'SHIFT_NAME', valueColumnName: 'SHIFT_NO'
				}, render: function (item){
					for (var i = 0; i < $shiftGroupList.length; i++){
						if ($shiftGroupList[i]['SHIFT_NO'] == item.SHIFT_NO)
							return $shiftGroupList[i]['SHIFT_NAME']
					}
					return item.SHIFT_NAME;
				}
			},
			{ display: '<spring:message code="hrms.description"/>', name: 'DESCR',width:160,
			...
			url: '/att/attendanceSettings/getShiftGroupInfoList',
			width: '99.6%', height: '100%',
	...
	function spf_showDetailData(row, detailPanel,callback){	
		...
		$(grid).css('margin',10).ligerGrid({
			columns:[
				...
				{ display: '<spring:message code="hrms.startedTime"/>', name: 'S_DATE',width:90},
				{ display: '<spring:message code="hrms.endTime"/>', name: 'E_DATE',width:90}
			], 
			usePager: false,rownumbers: true,
			isScroll: false, showToggleColBtn: false, width: '90%',
			url: '/att/attendanceSettings/getShiftGroupEmpInfoList', 
	...
	function spf_save(){
		...
		$.post("/att/attendanceSettings/addAndUpdateShiftGroupInfo", 
				[
	...
	function spf_update(){
		...
		$dialog = $.ligerDialog.open({isDrag: false, 
			...
			width: 960, 
			height: 500, 
			url: '/att/attendanceSettings/updateShiftGroupInfo?GROUP_NO=' + rows.GROUP_NO
		});
	}
	...
	function spf_delete(){
		$.ligerDialog.confirm('<spring:message code="hrms.whetherDelete"/>','<spring:message code="hrms.warning"/>', function (yes){
			if(yes){
				...
				$.post("/att/attendanceSettings/deleteShiftGroupInfo",
						[	
