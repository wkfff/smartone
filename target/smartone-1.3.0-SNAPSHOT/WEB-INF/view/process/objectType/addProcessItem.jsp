...
<script type="text/javascript">
    ...
    function spf_save() {
        if ($('#OBJECT_TYPE_NO').val() == null || $('#OBJECT_TYPE_NO').val() == '') {
            return;
        }
        if (spf_checkData()) {
    ...
    function spf_checkData() {
        if ($("#OBJECT_TYPE_NO").val() == null || $("#OBJECT_TYPE_NO").val() == "") {
            $.ligerMessageBox.warn('<spring:message code="hrms.warning"/>', 'Object类型' + ". " + '<spring:message code="input.required"/>');
            return false;
        }
        if ($("#STATUS_NO").val() == null || $("#STATUS_NO").val() == "") {
            $.ligerMessageBox.warn('<spring:message code="hrms.warning"/>', '状态' + ". " + '<spring:message code="input.required"/>');
            return false;
        }
        if ($("#STATUS_TYPE_CODE").val() == null || $("#STATUS_TYPE_CODE").val() == "") {
            $.ligerMessageBox.warn('<spring:message code="hrms.warning"/>', '状态类型' + ". " + '<spring:message code="input.required"/>');
            return false;
        }
        return true;
    }
    ...
    <div id="layout1" style="width: 99.6%; margin: 0; padding: 0;">
        ...
        <table width="99.6%" align="center" cellpadding="0" cellspacing="0" class="l-table-edit_01" border="1xp" bordercolor="#A3C0E8">
            <tr>
                ...
                <td width="35%" class="l-table-edit-c"><SipingSoft:selectStatusNo name="STATUS_NO" selected="${ObjectTypeDetail.state.no}" /></td>
                <td width="15%" class="l-table-edit-t">状态类型</td>
                <td width="35%" class="l-table-edit-c"><Static:stateType name="STATUS_TYPE_CODE" selected="${ObjectTypeDetail.stateType.id}" /></td>
            </tr>
            <tr>
                <td width="15%" class="l-table-edit-t">成功状态</td>
                <td width="35%" class="l-table-edit-c"><SipingSoft:selectStatusNo name="SUC_STATUS_NO" objectTypeNo="${paramMap.typeNo}" limit="all" /></td>
        ...
        <table width="99.6%" align="center" cellpadding="0" cellspacing="0" class="l-table-edit_01" border="1xp" bordercolor="#A3C0E8">
            <tr>
                ...
                <td width="20%" class="l-table-edit-c"><SipingSoft:selectStatusNo name="PRE_STATUS_NO1" objectTypeNo="${paramMap.typeNo}" limit="all" /></td>
                <td width="10%" class="l-table-edit-t">返回状态2</td>
                <td width="20%" class="l-table-edit-c"><SipingSoft:selectStatusNo name="PRE_STATUS_NO2" objectTypeNo="${paramMap.typeNo}" limit="all" /></td>
                <td width="10%" class="l-table-edit-t">返回状态3</td>
                <td width="20%" class="l-table-edit-c"><SipingSoft:selectStatusNo name="PRE_STATUS_NO3" objectTypeNo="${paramMap.typeNo}" limit="all" /></td>
            </tr>
        </table>
    </div>
</body>
