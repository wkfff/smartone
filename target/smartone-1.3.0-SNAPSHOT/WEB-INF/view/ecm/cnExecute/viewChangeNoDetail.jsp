...
<script type="text/javascript">
    ...
    function spf_updateECMObject() {
        var row = $curgrid.getSelectedRow();
        if (row == null || row == '') {
            $.ligerMessageBox.error('<spring:message code="hrms.warning"/>','<spring:message code="hrms.chooseLine"/>');
            return;
        }
        if (row.OBJECT_CATEGORY == 'MAT') {
            $.ligerMessageBox.warn('<spring:message code="hrms.warning"/>', '不可直接变更物料');
            return;
        }
        $.post('/ecm/cnExecute/updateECMObject', {
            'OBJECT_ID' : row.OBJECT_ID,
            'OBJECT_CATEGORY' : row.OBJECT_CATEGORY,
            'IS_UPDATED' : row.IS_UPDATED,
            'CHANGE_NO' : '${changeNo.no}',
            'S_DATE' : '${changeNo.startDate}',
            'E_DATE' : '${changeNo.endDate}'
        }, function(result) {
            tab.addTabItem({
                tabid : 'changeObj',
                text : '进行修改',
                url : result + '&S_DATE=${changeNoDetail.S_DATE}' + '&E_DATE=${changeNoDetail.E_DATE}'
            });
        });
    }
</script>
        ...
        <table width="99.6%" align="center" cellpadding="0" cellspacing="0" class="l-table-edit_01" border="1xp" bordercolor="#A3C0E8">
            <tr>
                ...
                <td width="30%" class="l-table-edit-c">
                    ${changeNo.no}</td>
                <td width="20%" class="l-table-edit-t"><spring:message code="hrms.documentNumber" /></td>
            ...
            <table width="99.6%" cellpadding="1" cellspacing="1"
                ...
                <tr>
                    ...
                    <td align="right" height="30px;"><a class="l-button"
                        ...
                        onclick="spf_updateECMObject()">变更</a></td>
                </tr>
            </table>
