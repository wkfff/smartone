...
<script type="text/javascript">
    ...
    function f_initGrid(){
        $grid = $("#empInfo").ligerGrid({
            ...
            columns: [
                        ...
                        { display: '<spring:message code="hrms.documentNumber"/>', name: 'TRANS_NO', width: 10,hide:1},
                        ...
                        { display: '<spring:message code="hrms.EmpStatus"/>', name: 'EMPSTATUS', width: 120},
                        { display: '职位等级', name: 'POST_LEVEL_NAME', width: 120},
            ...
            url: '/hrm/searchTransferOrder/getExpInsideInfoList',
            parms: [
                    { name: 'TRANS_CODE',value: 'TransCode_110'}
                   ],
    ...
    function spf_delete(){
        ...
        $(rows).each(function (index, row){
           ...
           if(this.CANCEL_REMARK == null || this.CANCEL_REMARK == 'undefined' ||this.CANCEL_REMARK == ''){
        ...
        $.ligerDialog.confirm('您确定要删除该条申请?','<spring:message code="hrms.warning"/>', function (yes){
            if(yes){
                ...
                [
                    ...
                    { name: 'rollbackType', value: 'normal' }
                ],
                function (result){
    ...
    function spf_insExcel(colums){
        ...
        var trans_code = '${TRANS_CODE}';
        var affirm_flag = $("#AFFIRM_FLAG").attr('value');
        var executed = 1;
        if (s_date == '' || e_date == '') {
            $.ligerMessageBox.warn('<spring:message code="hrms.warning"/>', '请在"生效日期"选择正确的起止时间');
            return ;
        }
        var attMonth = getformatdate(s_date);
        location.href='/hrm/transferOrder/getTransferOrderByExcel?PA_MONTH=' + attMonth+'&S_DATE='
                +s_date+'&E_DATE='+e_date+'&TRANS_CODE='+trans_code+'&affirm_flag='+affirm_flag + '&EXECUTED='+executed
                +'&sheetTitle=职员调动信息';
            ...
            <div position="top">
                <table width="99.6%" cellpadding="0" cellspacing="0" class="l-table-edit_01" border="1xp" bordercolor="#A3C0E8">
                    <tr>
                        ...
                        <td width="35%" class="l-table-edit-c"><SipingSoft:deptTree name="DEPTID" limit="hr" /></td>
                    </tr>
                    <tr>
                        <td class="l-table-edit-t"><spring:message code="hrms.active" /></td>
                        <td class="l-table-edit-c"><SipingSoft:selectOrderStatus name="EXECUTED" limit="all" selected="" onChange="spf_search();" /></td>
                        <td class="l-table-edit-t" nowrap="nowrap"><spring:message code="hrms.approvalActive" /></td>
                        <td class="l-table-edit-c" nowrap="nowrap"><SipingSoft:selectAppStatus name="AFFIRM_FLAG" limit="all" onChange="spf_search();" /></td>
                    </tr>
                    <tr>
                        <td class="l-table-edit-t"><spring:message code="hrms.effectDate" /></td>
                ...
                <table cellpadding="0" cellspacing="0" class="l-table-edit" height="30">
                    <tr>
                        <td align="left">
                            <SipingSoft:button />
                            <!-- <a class="l-button" id="btn" style="width: 79px; height: 20px; float: right; margin-left: 10px; " onclick="spf_insExcel()">Excel导出</a> -->
                        </td>
                    </tr>
                </table>
            </div>
