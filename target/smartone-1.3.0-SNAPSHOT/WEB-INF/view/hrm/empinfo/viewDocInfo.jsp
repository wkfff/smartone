	    ...
	    $(function ()
	   {
	    ...
	    function f_initGrid()
	    {
	    	$grid = $("#workExperienceInfo").ligerGrid({
	    		checkbox: false,
	            columns: [ 
	           	    { display: '', name: 'DOC_NO',width: 10,hide:1},  
	           	    { display: '<spring:message code="hrms.documentType"/>', name: 'DOC_TYPE_NAME', width: 160},
	           	    { display: '<spring:message code="hrms.startDate"/>', name: 'START_DATE', width: 160},	           	    
					{ display: '<spring:message code="hrms.endDate"/>', name: 'END_DATE', width: 160},
					{ display: '<spring:message code="hrms.documentID"/>', name: 'DOC_ID', width: 130}, 
					{ display: '<spring:message code="hrms.evidenceDate"/>', name: 'OBTAINED_DATE', width: 120}, 
					{ display: '<spring:message code="hrms.signDept"/>', name: 'SIGN_DEPT', width: 110},  
					{ display: '<spring:message code="hrms.remarks"/>', name: 'REMARK', width: 110}
	            ], 
	            enabledEdit: false, usePager: false,rownumbers:true, 
                url: "/hrm/empinfo/getDocInfoList?EMPID=${basicInfo.EMPID }", 
                pageSize: 5,	    
	    ...
	    function spf_add()
	    {
	    	$dialog = $.ligerDialog.open({isDrag: false, showMax:false,showMin:false,
	    		...
	    		url: '/hrm/empinfo/addDocInfo?EMPID=${basicInfo.EMPID}'
	        	  });
	    }
	    function spf_update()
	    { 
	    	...
	    	$dialog = $.ligerDialog.open({isDrag: false, 
	    		...
	    		width: 800, height: 400, 
	    		url: '/hrm/empinfo/updateDocInfo?DOC_NO=' + rows.DOC_NO
	       	  });
	    }
	    function spf_delete()
	     {        
	     	$.ligerDialog.confirm('<spring:message code="hrms.whetherDelete"/>','<spring:message code="hrms.warning"/>', function (yes)
	         {
	         	if(yes){
	 		        ...
	 		        $.post("/hrm/empinfo/deleteDocInfo", 
	 		                [	
	 		                 	{ name: 'DOC_NO', value: rows.DOC_NO }
	 		                ]
	 		        , function (result)
	 		        {
				...
				<table width="99.5%" cellpadding="1" cellspacing="1" style="height:25px;font-size: 12px;">
					<tr>
						...
						<td width="25%" align="left" style="font-weight: bold;">
							<spring:message code="hrms.documentInfo"/>
						</td>	
