...
<script type="text/javascript">
	...
	$(function (){ 
		...
		$(paColumnsList).each(function(index, row){  
			if(row.COLUMN_NAME=="EMPID"||row.COLUMN_NAME=="CHINESENAME"||row.COLUMN_NAME=="DEPTNAME"){
				colums.push({ display: row.ITEM_NAME, name: row.COLUMN_NAME , width:100 ,frozen:true }) ;
			}else{
				colums.push({ display: row.ITEM_NAME, name: row.COLUMN_NAME , width:90 }) ;
			}
		}); 
		spf_initGrid(colums); 
	});
							...
							<table width="99.6%" cellpadding="0" cellspacing="0"
								...
								<tr>
									...
									<td width="85%" class="l-table-edit-c">
										<SipingSoft:date yearName="year" monthName="month" onChange="spf_search();" />
									</td>
								</tr>
							</table>
						</td>
