    ...
    $(function ()
   {
    ...
    function f_initGrid()
    {
    	$grid = $("#channelInfo").ligerGrid({
    		//checkbox: true,   		
    		columns: [   
			            { display: '经销商编号', name: 'CHANNEL_ID', width: 90},
			            { display: '经销商名称', name: 'CHANNEL_NAME', width: 90},
						{ display: '等级', name: 'CHA_LEVEL', width: 90},
			            { display: '总信用额度', name: 'TOTAL_CREDIT', width: 90},
			            { display: '剩余额度', name: 'LEFT_CREDIT', width: 90}
		            ],
            enabledEdit: true,usePager: true, rownumbers:true,
            url: '/utility/chaInfo/getChannelList',
            /*parms: [	
     ...
     <div position="center" id='' title=' ' style="border: 0px;"> 
		<div position="top">
			<table width="99.6%" cellpadding="0" cellspacing="0"
				...
				<tr>
					<td width="15%" class="l-table-edit-t">经销商名称<spring:message
							code="keyWord" /></td>
				...
				<tr>
				<td>
					<a class="l-button" style="width:79px; height:20px; float:left; margin-left:10px;" onclick="spf_search()">删除</a>
				</td>	
				</tr>
			</table>
		</div>		 
