...
<script type="text/javascript"> 
	...
	function f_initGrid()
	{
		$grid = $("#empInfo").ligerGrid({
			...
			{ display: '<spring:message code="hrms.workingContent"/>', name: 'BT_CONTENT', align: 'left', width: 120,
				editor: { type: 'textarea', height: 100 }
			},
			{ display: '<spring:message code="hrms.remarks"/>', name: 'BT_REMARK', align: 'left', width: 120,
				editor: { type: 'textarea', height: 100 }
			},
		...
		if(item.APP_STATUS_ID == 0 && item.AFFIRM_FLAG == 1){  
			...
			url: '/ess/affirmApp/getAffirmBTInfo',
			parms:[	
	...
	{    
		$dialog = $.ligerDialog.open({isDrag: false, 
			...
			width: 700, 
			height: 320, 
				url: '/ess/viewApp/viewApproverInfo?PAGE=A&FLAG='+flag+'&APPLY_TYPE=BTApply&APPLY_NO='+applyNo
				});
	}
	function spf_search()
	{
