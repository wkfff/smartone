...
<script type="text/javascript"> 
    var $grid = null ; 
    //扩展一个 多行文本框 的编辑器
    $.ligerDefaults.Grid.editors['textarea'] = {create: function (container, editParm){
    ...
    function f_initGrid(){
    	$grid = $("#empInfo").ligerGrid({
    		...
    		columns: [
			            ...
			            { display: '<spring:message code="hrms.expType"/>', name: 'EXP_TYPE_NAME', width: 90},
			            { display: '<spring:message code="hrms.amountOfMoney"/>', name: 'MONEYAMOUNT', width: 90},
			            { display: '<spring:message code="hrms.applicationDate"/>', name: 'EXP_DATE', width: 110},
			            { display: '<spring:message code="hrms.applyPurpose"/>', name: 'APPLYPURPOSE', align: 'left', width: 300,
...
//            }
    ...
    function spf_showExpDetail(flag,applyNo){
    	$dialog = $.ligerWindow.show({isDrag: true,
		    title: '<spring:message code="hrms.expDetail"/>',
      	 	width: 900 ,
      	 	height: 450,
      	 	left:120,
      	 	top:60,
            url:'/ess/viewApp/viewExpDetailInfo?EXP_NO=' + applyNo + '&FLAG=' + flag,
      	});
    }
    ...
    function spf_showDetailData(applyNo){
    	$dialog = $.ligerDialog.open({isDrag: false, 
    		...
    		width: 700, 
    		height: 300, 
  		    url: '/ess/viewApp/viewApproverInfo?PAGE=B&FLAG=0&APPLY_TYPE=ExpApply&APPLY_NO='+applyNo
       	  });
    }
    function spf_search(){
		...
		<div position="center" id='' title=' '>
			<div position="top">
				<table width="99.6%" cellpadding="0" cellspacing="0"
					...
					<tr>
						<td class="l-table-edit-t"><spring:message code="hrms.expType" /></td>
						...
						<td class="l-table-edit-t"><spring:message code="hrms.approvalActive" /></td>
