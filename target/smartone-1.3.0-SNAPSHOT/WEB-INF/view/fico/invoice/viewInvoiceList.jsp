...
<script type="text/javascript"> 
    ...
    function f_initGrid()
    {
    	$grid = $("#invoiceInfo").ligerGrid({
    		columns: [
			            { display: '款项支出编号', name: 'INVOICE_NO', width: 90, frozen:true },
			            { display: '主题', name: 'SUBJECT', width: 90, frozen:true},
						{ display: '账号', name: 'ACCOUNT_ID', width: 90},
						{ display: '应付金额', name: 'PAYABLE_AMOUNT', width: 90},
						{ display: '已付金额', name: 'PAID_AMOUNT', width: 90},
						{ display: '类型', name: 'TYPE_NAME', width: 90},
						{ display: '状态', name: 'STATUS_NAME', width:90},
						...
						{ display: '项目', name: 'PROJECT_NAME', width:90},
            ...
            pageSize:20,
            url: '/fico/invoice/getInvoiceList',
            parms: [
                   ],
    ...
    function spf_viewInvoiceDetail(INVOICE_NO){
    	if(tab.isTabItemExist('fico0102')){
    		tab.removeTabItem('fico0102');
    	}
        tab.addTabItem({ tabid : 'fico0102',text: '支出信息', url: '/fico/invoice/viewInvoiceDetail?MENU_CODE=fico0102&INVOICE_NO='+INVOICE_NO });
    }
    function spf_delete()
    {   
		...
		$dialog = $.ligerDialog.open({isDrag: false, 
     		...
     		width: 420, 
     		height: 220, 
     		url: '/fico/invoice/removeInvoice?INVOICE_NO='+row.INVOICE_NO
    	});
    }
    ...
    function spf_add(){
        ...
        tab.addTabItem({ tabid : 'fico0103',text: '添加支出', url: '/fico/invoice/addInvoice?MENU_CODE=fico0103' });
    }
    ...
    function spf_update(){
	    ...
	    tab.addTabItem({ tabid : 'fico0104',text: '编辑支出', url: '/fico/invoice/updateInvoice?MENU_CODE=fico0104&INVOICE_NO='+row.INVOICE_NO });
    }
    function spf_search()
    {
    	$grid.setOptions({ parms: [	
									...
									{ name: 'TYPE_ID', value: $("#TYPE_ID").attr('value')}
    	                           ],
    	                   newPage: 1
    	                 }); //设置数据参数
		...
		<div position="center" id='' title=' '>
			<div position="top">
				<table width="99.6%" cellpadding="0" cellspacing="0"
					...
					<tr>
						<td width="15%" class="l-table-edit-t">类型</td>
						<td width="35%" class="l-table-edit-c">
							<SipingSoft:selectSyCode name="TYPE_ID" parentCode="invoiceTypeCode" onChange="spf_search()" limit="all"/>
						</td>
