...
<script type="text/javascript">
	...
	function f_save() {
		if (spf_checkData()) {
			...
			var options = {
				url : '/pa/salary/uploadPaParamExcel',
				type : 'POST',
	...
	function spf_checkData() {
		if ($("#file").val() == null || $("#file").val() == "") {
			$.ligerMessageBox.warn('<spring:message code="hrms.warning"/>',
					'<spring:message code="hrms.file"/>' + ". "+ '<spring:message code="input.required"/>');
			return false;
		}
		var fileName = $("#file").val();
		var fileType = fileName.substring(fileName.lastIndexOf(".") + 1).toUpperCase();
		if (fileType != "XLS") {
			$.ligerMessageBox.warn('<spring:message code="hrms.warning"/>','文件类型必须为.xls');
			return false;
		}
		return true;
	}
...
<div id="layout1"> 
	<form name="form1" method="post" action="" id="form1"
		...
		<input type="hidden" id="PA_MONTH" name="PA_MONTH" value="${paraMap.PA_MONTH}"/>
		<input type="hidden" id="PARAM_NO" name="PARAM_NO" value="${paraMap.PARAM_NO}"/>
		<div id="inputID">
							...
							<tr>
								...
								<td height="30px;">
									<div align="left" style="font-weight: bold;">
										<a style="color: #ff0000">提示：上传数据表单格式请以模板格式为准。</a>
									</div></td>
							</tr>
						</table></td>
				</tr>
