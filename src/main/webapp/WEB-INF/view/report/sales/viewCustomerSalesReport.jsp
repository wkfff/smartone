...
<script type="text/javascript">
    ...
    $(function() {
        ...
        refreshDataM();
        refreshDataY();
        //$("#customerMonthlyTable").tablesorter();
        //$("#customerYearlyTable").tablesorter();
        ...
        $('#customerMonthlyTable th').click(function(c){
            if($(this).attr("class")=="desc"){
                ...
                if($(this).attr("class")=="sc"){
                    ...
                    $("#customerMonthlyTable th").removeClass("desc");
                    $("#customerMonthlyTable th").removeClass("sc");
                    $(this).addClass("desc");
                }
            }
            refreshDataM();
        });
        $('#customerYearlyTable th').click(function(c){
            if($(this).attr("class")=="desc"){
                $(this).attr("class","sc");
            }else{
                if($(this).attr("class")=="sc"){
                    ...
                    $("#customerYearlyTable th").removeClass("desc");
                    $("#customerYearlyTable th").removeClass("sc");
                    $(this).addClass("desc");
                }
            }
            refreshDataY();
    ...
    function refreshDataM() {
        ...
        var orderby = "";
        if($("#customerMonthlyTable .desc").attr("id")!=undefined){
            orderby = $("#customerMonthlyTable .desc").attr("id") + " "
                +$("#customerMonthlyTable .desc").attr("class");
        }
        if($("#customerMonthlyTable .sc").attr("id")!=undefined){
            orderby = $("#customerMonthlyTable .sc").attr("id");
        }
        
        $.ajax({
            type : "post",
            dataType : "json",
            url : '/report/sales/getCustomerMonthlySalesList',
            ...
            success : function(data) {
                ...
                for ( var i = 0; i < data.Rows.length; i++) {
                    total = 0;
                    innerHtml.push("<tr>");
                    if(data.Rows[i].CUST_NAME.length > 9){
                        innerHtml.push("<td class='t1'>" + data.Rows[i].CUST_NAME.substr(0,8)+ "..</td>");
                    }
                    else{
                        innerHtml.push("<td class='t1'>" + data.Rows[i].CUST_NAME+ "</td>");
                    }
                ...
                //$("#customerMonthlyTable").trigger("update");
    ...
    function refreshDataY() {
        var orderby = "";
        if($("#customerYearlyTable .desc").attr("id")!=undefined){
            orderby = $("#customerYearlyTable .desc").attr("id") + " "
                +$("#customerYearlyTable .desc").attr("class")
        }
        if($("#customerYearlyTable .sc").attr("id")!=undefined){
            orderby = $("#customerYearlyTable .sc").attr("id");
        }
        $.ajax({
            type : "post",
            dataType : "json",
            url : '/report/sales/getCustomerYearlySalesList',
            ...
            success : function(data) {
                ...
                for ( var i = 0; i < data.Rows.length; i++) {
                    total = 0;
                    innerHtml.push("<tr>");
                    if(data.Rows[i].CUST_NAME.length > 18){
                        innerHtml.push("<td class='t1'>" + data.Rows[i].CUST_NAME.substr(0,17)+ "..</td>");
                    }
                    else{
                        innerHtml.push("<td class='t1'>" + data.Rows[i].CUST_NAME+ "</td>");
                    }
                ...
                //$("#customerYearlyTable").trigger("update");
                totalPages = parseInt(data.Total/pageSize);
                if((data.Total-totalPages*pageSize)>0){
                    totalPages ++;
                }
...
#customerSalesMonth .t1,#customerSalesYear .t1 {
    background: yellow
}

#customerSalesMonth .desc, #customerSalesYear .desc {
    background-color: #ff0000
}

#customerSalesMonth .sc, #customerSalesYear .sc {
...
<body style="padding: 2px">
    ...
    <div id="layout1" style="margin-left:20px">
        ...
        <div id='MONTHLYDiv'>
            ...
            <br />
            <div id='customerSalesMonth' style='height:400px; width:95%'>
                <table id="customerMonthlyTable">
                    <thead>
                        <tr>
                            <th style="width:79px;" id="CUST_NAME">
                                <div id="out">
                                    <div id="up"><spring:message code="report.month"/></div><div id="down"><spring:message code="hrms.customer"/></div>
                                </div>
                            </th>
        ...
        <div id='YEARLYDiv'>
            <br />
            <div id='customerSalesYear' style='height:400px; width:95%'>
                <table id="customerYearlyTable">
                    <thead>
                        <tr>
                            <th style="width:80px;" id="CUST_NAME">
                                <div id="out">
                                    <div id="up"><spring:message code="report.years"/></div><div id="down"><spring:message code="hrms.customer"/></div>
                                </div>
                            </th>
