...
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
 ...
 <head>   
   <meta http-equiv="X-UA-Compatible" content="IE=edge" >
    ...
    function f_initGrid()
    {
    	$grid = $("#appProcessPInfo").ligerGrid({
    		checkbox: false,
            columns: [
	        ...
	        { display: '<spring:message code="hrms.setApprover"/>', name: 'SET_APP_NAME', align: 'center'}
            ],
            usePager: true, dataAction: 'server', root: 'appProcessPList', record: 'appProcessPCnt',
            ...
            columns:
                        [
                         { display: '<spring:message code="hrms.approvalType"/>', name: 'AFFIRM_TYPE_NAME' , align: 'center'}, 
         				 ...
         				 { display: '<spring:message code="hrms.approvalType"/>', name: 'EMPID',hide:1},
                        ...
                        usePager: false,rownumbers: true,dataAction: 'server', root: 'appProcessPAList',
                        isScroll: false, showToggleColBtn: false, width: '90%',
                        url: '/system/appProcessManagement/getAppProcessPAInfo?EMPID='+row.EMPID , 
                        showTitle: false, columnWidth: 100, 
             			onAfterShowData: callback,allowUnSelectRow:true,
                        onSelectRow: function (data, rowindex, rowobj)
                        { 
                            spf_delete(data.EMPID,data.AFFIRM_TYPE_ID)
                        }
        });
    }
    ...
    { 
    	...
    	$dialog = $.ligerDialog.open({isDrag: false, title:'<spring:message code="hrms.update"/>', 
    		width: $("#layout1").width(), height: $("#layout1").height(), url: '/system/rightsManagement/updateLoginUserView?EMPID=' + rows.EMPID
       	  });
    }
    ...
    { 
    	...
    	$dialog = $.ligerDialog.open({isDrag: false,  title:'<spring:message code="hrms.update"/>', 
    		width: $("#layout1").width(), height: $("#layout1").height(), url: '/system/appProcessManagement/addAppProcessPInfo?EMPID=' + rows.EMPID
       	  });
    }
   

    function spf_delete(EMPID,AFFIRM_TYPE_ID)
    {  
    	$dialog = $.ligerDialog.open({isDrag: false,  title:'<spring:message code="hrms.delete"/>', 
    		width: $("#layout1").width()-40, height: $("#layout1").height()-40, url: '/system/appProcessManagement/deleteAppProcessPInfo?AFFIRM_OBJECT='+EMPID+'&AFFIRM_TYPE_ID='+AFFIRM_TYPE_ID+'&EMPID='+EMPID
       	  });
    }
...
<body style="padding:2px" > 
	<div id="layout1" > 
      ...
      <div position="center" id='appProcessPInfo' ></div> 
 </div>        
</body>
</html>
