...
<script type="text/javascript">
    ...
    $(function ()
    {
        //布局
         ...
         $otTypeSelectList = getOTTypeSelectList() ;  
    ...
    function getOTTypeSelectList(){
    	//请求服务器
    	var otTypeSelectList ;
        $.ajax({
            type: 'post',
            url: '/utility/otherInfo/getOTTypeSelectList', 
            dataType: 'json',
            async: false,
            success: function (list)
            {  
            	otTypeSelectList = list ;
            }
        });   
        return otTypeSelectList ;
    }
    ...
    {
    	$grid = $("#otParameterInfo").ligerGrid({
    	 checkbox: false,   
            columns: [
            { display: '<spring:message code="hrms.primaryKey"/>', name: 'PARAMETER_NO', width: 80},
            { display: '<spring:message code="hrms.otType"/>', name: 'OT_TYPE_CODE', width: 130,
                editor: { 
                	type: 'select', data: $otTypeSelectList, dataValueField: 'OT_TYPE_CODE',dataDisplayField: 'OT_TYPE_NAME', 
                	displayColumnName: 'OT_TYPE_NAME', valueColumnName: 'OT_TYPE_CODE'
                }, render: function (item)
                {
                    for (var i = 0; i < $otTypeSelectList.length; i++)
                    {
                        if ($otTypeSelectList[i]['OT_TYPE_CODE'] == item.OT_TYPE_CODE)
                            return $otTypeSelectList[i]['OT_TYPE_NAME']
                    }
                    return item.OT_TYPE_NAME;
                }
	        },
	        { display: '<spring:message code="hrms.otLength"/>', columns:
                [
                 { display: '<spring:message code="hrms.startedHours"/>', name: 'OT_LENGTH_S',width: 120,type: 'int', editor: { type: 'int' }},
                 { display: '<spring:message code="hrms.endHours"/>', name: 'OT_LENGTH_E',width: 120,type: 'int',editor: { type: 'int' }}
                ]
            }, 
            ...
            enabledEdit: true,usePager: true, rownumbers: true,dataAction: 'server', root: 'otAppParameterInfoList', record: 'otAppParameterInfoCnt',
            url: '/system/systemParameter/getOTAppParameterInfo',
    ...
    {  
        ...
        $.post("/system/systemParameter/saveAndUpdateOTAppParameterInfo", 
                [	
    ...
    {       
    	$.ligerDialog.confirm('<spring:message code="hrms.whetherDelete"/>','<spring:message code="hrms.warning"/>', function (yes)
        {
        	if(yes){
		        ...
		        $.post("/system/systemParameter/deleteOTAppParameterInfo", 
		                [	
...
<body style="padding:2px"> 
      <div id="layout1" > 
		  <div position="top">
		                ...
		                <td align="center" valign="middle"> 
		            		<table width="99.6%" cellpadding="0" cellspacing="0" class="l-table-edit_01" border="1xp" bordercolor="#A3C0E8">	            
				                <tr>
					               ...
					               <td class="l-table-edit-t"><spring:message code="hrms.otType"/></td>
					           	   <td class="l-table-edit-c">
										<SipingSoft:selectSyCode parentCode="OTTypeCode" name="OT_TYPE_CODE" limit="all"/>
						       	   </td>
               ...
               <div position="center" id="otParameterInfo" ></div>
</div>  
</body>
</html>
