...
function spf_dg_initGrid(){
	$dg_grid = $("#dgEmpInfo").ligerGrid({
		...
		columns: [
				...
				{ display: '开始时间', name: 'S_DATE',width:90},
				{ display: '结束时间', name: 'E_DATE',width:90}
		],
		usePager: true,
		url: '/att/attendanceSettings/getShiftGroupEmpInfoList', 
		parms: [
				{ name: 'GROUP_NO', value: '${shiftGroupInfo.GROUP_NO}'},
...
function spf_emp_initGrid(){
	$emp_grid = $("#empInfo").ligerGrid({
		...
		parms: [/* { name: 'GROUP_NOT_NO', value: '${shiftGroupInfo.GROUP_NO}'}, */
				{ name: 'keyWord', value: $("#keyWord").attr('value')},
				{ name: 'STATUS_CODE', value: $("#STATUS_CODE").val()},
				{ name: 'DEPTID', value: $("#DEPTID").attr('value')},
				{ name: 'NOT_DEPT',value: 'YES'}
		],
		width: '99%',height:'98%'
	});
}
...
function spf_delete(){
	...
	$.ligerDialog.confirm('<spring:message code="hrms.whetherDelete"/>','<spring:message code="hrms.warning"/>', function (yes){
		if(yes){
			...
			$.post("/att/attendanceSettings/deleteShiftGroupEmpInfo", 
				[	
...
function spf_save(){
	...
	if (sdate >= edate){
		$.ligerMessageBox.error('<spring:message code="hrms.warning"/>', '开始时间大于等于结束时间!') ;
		return;
	}
	if (!rows || rows.length == 0) { 
	$.ligerMessageBox.error('<spring:message code="hrms.warning"/>', '<spring:message code="hrms.chooseLine"/>') ; 
	...
	$(rows).each(function (){
		...
		jsonData += ' "GROUP_NO": "' +'${shiftGroupInfo.GROUP_NO}' + '", ' ;
		jsonData += ' "EMPID": "' + this.EMPID + '", ' ;
		jsonData += ' "S_DATE": "' + $("#S_DATE").val() + '", ' ;
		jsonData += ' "E_DATE": "' + $("#E_DATE").val() + '" ' ;
		jsonData += '}' ;
	}); 
	jsonData += ']' ;

	$.ligerDialog.waitting('<spring:message code="hrms.deleting"/>');
	$.post("/att/attendanceSettings/addGroupEmpInfo", 
		[	
			{ name: 'jsonData', value: jsonData }
		]
	, function (result){
