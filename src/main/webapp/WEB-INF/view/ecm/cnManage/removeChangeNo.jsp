...
<script type="text/javascript">
	...
	function spf_remove() {
		if (spf_checkData()) {
			...
			$.post("/ecm/cnManage/removeChangeNo", [ {
				name : 'REMOVE_REASON',
				value : $("#REMOVE_REASON").val()
			}, {
				name : 'CHANGE_NO',
				value : $("#CHANGE_NO").val()
			} ], function(result) {
	...
	function spf_checkData() {
		...
		if ($("#CHANGE_NO").val() == null || $("#CHANGE_NO").val() == "") {
			$.ligerMessageBox.warn('<spring:message code="hrms.warning"/>',
					'经销商编号' + ". " + '<spring:message code="input.required"/>');
			return false;
		}
		if ($("#REMOVE_REASON").val() == null
				|| $("#REMOVE_REASON").val() == "") {
			$.ligerMessageBox.warn('<spring:message code="hrms.warning"/>',
					'删除备注' + ". " + '<spring:message code="input.required"/>');
			return false;
		}
		return true;
	}
...
<body style="padding: 2px">
	<div id="layout1" style="width: 99.6%; margin: 0; padding: 0;">
		<input type="hidden" id="CHANGE_NO" name="CHANGE_NO"
			value='${changeNoDetail.CHANGE_NO}' />
		<table width="99.6%" align="center" cellpadding="0" cellspacing="0"
			...
			<tr>
				<td width="100%" class="l-table-edit-c" colspan="5" nowrap="nowrap">
					<textarea rows="3" cols="60" id="REMOVE_REASON"
						name="REMOVE_REASON"></textarea>
				</td>
			</tr>
			<tr>
				<td><a class="l-button"
					style="width: 79px; height: 20px; float: left; margin-left: 1px; "
					onclick="spf_remove()">删除</a></td>
			</tr>
		</table>
	</div>
</body>
