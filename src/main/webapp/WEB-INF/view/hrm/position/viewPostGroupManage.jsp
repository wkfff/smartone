...
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
   ...
   // 初始调用
    ...
    { 
    	$grid = $("#gridInfo").ligerGrid({
    		checkbox: false,
            columns: [
	            { display: '<spring:message code="hrms.primaryKey"/>', name: 'POST_GROUP_NO', width: 90,align: 'center', type: 'int',hide:1},
	            { display: '<spring:message code="hrms.basicCode"/>', name: 'POST_GROUP_ID', align: 'center'},
	            { display: '<spring:message code="hrms.chineseName"/>', name: 'POST_GROUP_NAME', align: 'center', editor: { type: 'text' }},
	            { display: '<spring:message code="hrms.englishName"/>', name: 'POST_GROUP_EN_NAME', align: 'center', editor: { type: 'text' }},
	            { display: '<spring:message code="hrms.description"/>', name: 'DESCRIPTION', width: 180, align: 'left', editor:{type:'textarea', height: 100}},
            ...
            url: '/hrm/position/getPostGroupInfo',
    ...
    function spf_save(){ 
        ...
        $.post("/hrm/position/updatePostGroupInfo", 
                [	
 ...
 ; 
		        ...
		        $.post("/hrm/position/deletePostGroupInfo", 
		                [	
		                 	{ name: 'POST_GROUP_NO', value: rows.POST_GROUP_NO },
		                 	{ name: 'POST_GROUP_ID', value: rows.POST_GROUP_ID }
		                ]
		        , function (result)
		        {
		        	$.ligerDialog.closeWaitting();
     ...
     function spf_add(){ 
    	...
    	$dialog = $.ligerDialog.open({isDrag: false, title:'<spring:message code="hrms.add"/>', 
    		 width: 800, height: 260, url: '/hrm/position/addPostGroupInfo'
       	  });
    }
	</script>
