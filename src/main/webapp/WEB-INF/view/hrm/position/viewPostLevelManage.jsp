   ...
   // 初始调用
    ...
    { 
        $grid = $("#gridInfo").ligerGrid({
            checkbox: false,
            columns: [
                { display: '<spring:message code="hrms.primaryKey"/>', name: 'POST_GRADE_NO', width: 90,align: 'center', type: 'int',hide:1},
                { display: '<spring:message code="hrms.basicCode"/>', name: 'POST_GRADE_ID', align: 'center'},
                { display: '<spring:message code="hrms.chineseName"/>', name: 'POST_GRADE_NAME', align: 'cenetr', editor: { type: 'text' }},
                { display: '<spring:message code="hrms.englishName"/>', name: 'POST_GRADE_EN_NAME', align: 'center', editor: { type: 'text' }},
            ...
            url: '/hrm/position/getPostLevelInfo',
    ...
    function spf_save(){
        ...
        $.post("/hrm/position/updatePostLevelInfo",
                [
    ...
    function spf_delete()
    {
        $.ligerDialog.confirm('<spring:message code="hrms.whetherDelete"/>','<spring:message code="hrms.warning"/>', function (yes)
        {
            if(yes){
                ...
                $.post("/hrm/position/deletePostLevelInfo", 
                        [   
                            { name: 'POST_GRADE_NO', value: rows.POST_GRADE_NO },
                            { name: 'POST_GRADE_ID', value: rows.POST_GRADE_ID }
                        ]
                , function (result)
                {
                    $.ligerDialog.closeWaitting();
     ...
     function spf_add(){
        $dialog = $.ligerDialog.open({isDrag: false, title:'<spring:message code="hrms.add"/>', 
             width: 800, height: 260, url: '/hrm/position/addPostLevelInfo'
          });
    }
    </script>
