...
<script type="text/javascript"> 
    ...
    function f_initGrid()
    {
        $grid = $("#customerInfo").ligerGrid({
            ...
            columns: [   
                        ...
                        { display: '<spring:message code="htms.activityContent"/>', name: 'CUST_ACTION_CONTENT', width: 440, editor:{type:'textarea', height: 100}},
                        { display: '<spring:message code="cust.eventsList"/>',  width: 90,
                            render: function (item)  
                            { 
                               return '<div style="cursor: pointer;" onClick="spf_showDetail(\''+item.CUST_ID+'\')"><spring:message code="ass.clickHereToView"/></div>';
                            }
                        }
                    ],
            ...
            parms: [    
                    ...
                    { name: 'ACTION_TYPE_CODE', value: 'ActionType001'}
                   ],
    ...
    function spf_showDetail(CUST_ID){
        ...
        tab.addTabItem({ tabid : 'cust0501',text: '<spring:message code="cust.eventsList"/>', url: '/cust/custAction/viewCustomerAction?MENU_CODE=cust0501&ACTION_TYPE_CODE=ActionType001&CUST_ID='+CUST_ID });
    }
    function spf_search()
    {
                ...
                <table width="99.6%" cellpadding="0" cellspacing="0" class="l-table-edit_01" border="1xp" bordercolor="#A3C0E8">
                    <tr>
                        ...
                        <c:if test="${isSupervisor}">
                            ...
                            <td width="35%" class="l-table-edit-c"><input type="text" id="EMP_NAME" size="30" onclick="searchEmp()" /> <input type="hidden" id="EMPID" size="30" /></td>
                        </c:if>
                        <c:if test="${!isSupervisor}">
                            <td width="15%" class="l-table-edit-t"><spring:message code="hrms.theStaff"/></td>
                            <td width="35%" class="l-table-edit-c"><input name="EMP_ID" type="text" id="EMP_ID" size="30" readonly="readonly" value="${admin.adminID}" /></td>
                        </c:if>
                    </tr>
                    <tr>
                        ...
                        <td width="35%" class="l-table-edit-c"><SipingSoft:selectSyCode parentCode="CustActionWayCode" name="CUST_ACTION_WAY_CODE" onChange="spf_search();" limit="all" /></td>
                    </tr>
                    <tr>
                        ...
                        <td width="35%" class="l-table-edit-c"><SipingSoft:selectSyCode parentCode="CustActionStateCode" name="CUST_ACTION_STATE_CODE" onChange="spf_search();" limit="all" /></td>
                    </tr>
                </table>
