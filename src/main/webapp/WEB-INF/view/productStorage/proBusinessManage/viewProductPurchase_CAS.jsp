...
<script type="text/javascript">
                ...
                if(no<101){
                    ...
                    for(var i=0;i<columns;i++){
                        if(i==0){
                            ...
                            tr=tr+"<td width='45'><input type='text' class='tableInput' "+" name='productCAS"+no+"' id='productCAS"+no+"' readonly='readonly'/></td>";
                        }else if(i==4){
    ...
    //审批人选择 
    $(function (){
        $("#CHECKER_NAME").ligerComboBox({
            onBeforeOpen: spf_selectChecker, valueFieldID: 'EMPID',width:150
        });
    });
    function spf_selectChecker(){
        $.ligerDialog.open({
            title: '<spring:message code="employee.information"/>', 
            name:'winselector'+Math.random(),
            width: 800, 
            height: 400, 
            url: '/utility/empInfo/viewHrSearchEmployeeC', 
            buttons: [
                        { text: '<spring:message code="okay"/>', onclick: spf_selectCheckerOK },
                        { text: '<spring:message code="cancel"/>', onclick: spf_selectCheckerCancel },
                        { text: '<spring:message code="clear"/>', onclick: spf_selectCheckerClear }
                    ]
        });
        return false;
    }
    function spf_selectCheckerOK(item, dialog){
        var fn = dialog.frame.spf_selectEmpInfo || dialog.frame.window.spf_selectEmpInfo; 
        var data = fn(); 

        $("#CHECKER_NAME").val(data.CHINESENAME);
        $("#CHECKER_ID").val(data.EMPID);  
        dialog.close();
    }
    function spf_selectCheckerCancel(item, dialog){
        dialog.close();
    }
    function spf_selectCheckerClear(item, dialog){
        $("#CHECKER_NAME").val("");
        $("#CHECKER_ID").val(""); 
        dialog.close();
    }
            ...
            <div>
                ...
                <table width="99%" align="center" cellpadding="0"
                    ...
                    <tr>
                        ...
                        <td class="l-table-edit-c" width="35%">
                            <input width="120" name="CHECKER_NAME" id="CHECKER_NAME" type="text" class="inputClass" style="height:20px;"/>
                    ...
                    <tr>
                        ...
                        <td class="l-table-edit-c" colspan="3"><textarea class="inputClass" style="width: 390px; height: 50px" id="REMARKS"
