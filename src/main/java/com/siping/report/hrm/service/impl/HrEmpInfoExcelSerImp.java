...
@Service
public class HrEmpInfoExcelSerImp implements HrEmpInfoExcelSer {

	Logger logger = Logger.getLogger(HrEmpInfoExcelSerImp.class);

	@Autowired
	private HrEmpInfoReportDao hrEmpInfoReportDao;
	...
	public void viewEmpInfoReport(HttpServletRequest request,
		...
		// 页面提交的JSON信息
		String jsonCodeString = hrEmpInfoReportDao.getEmpInfoColumnInfo(paramMap); 
		...
		List<String> titleList = this.hrEmpInfoReportDao
				.getEmpInfoTitleInfo(paramMap);
		...
		Layouter.buildEmpInfoReport(worksheet, startRowIndex, startColIndex, titleList);
		...
		try {
			fileName = java.net.URLEncoder.encode("数据信息", "UTF-8");
	...
	private List<LinkedHashMap> getDatasource(List<String> codeList, Object obj) {
		return hrEmpInfoReportDao.getDatasource(codeList, obj);
	}
	...
	public int insertComputer(List<Map> list) {
		return hrEmpInfoReportDao.insertComputer(list);
