package com.siping.report.pa.service.imp.bonus;
...
@Service
public class BonusExcelSerImp implements BonusExcelSer {

	Logger logger = Logger.getLogger(BonusExcelSerImp.class);
 
	@Autowired
	private BonusResultReportDao bonusResultReportDao;
	...
	public void viewBonusResultReport(HttpServletRequest request,
		...
		List<String> titleList = this.bonusResultReportDao
				.getBonusTitleInfo(paramMap);
		...
		Layouter.buildBonusReport(worksheet, startRowIndex, startColIndex, titleList);
		...
		try {
			fileName = java.net.URLEncoder.encode(paramMap.get("PA_MONTH").toString()+"月奖金数据信息", "UTF-8");
	 ...
	 */
	@SuppressWarnings({ "rawtypes" })
	private List<LinkedHashMap> getDatasource(List<String> list, Object obj) {
		return bonusResultReportDao.getDatasource(list, obj);
	}
	...
	public int insertComputer(List<Map> list) {
		return bonusResultReportDao.insertComputer(list);
