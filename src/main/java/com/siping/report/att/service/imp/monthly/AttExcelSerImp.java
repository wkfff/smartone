...
@Service
public class AttExcelSerImp implements AttExcelSer {

	Logger logger = Logger.getLogger(AttExcelSerImp.class);

	@Autowired
	private AttMonthReportDao attMonthReportDao;
	...
	public void viewAttMonthReport(HttpServletRequest request,
		...
		try {
			fileName = java.net.URLEncoder.encode(paramMap.get("ATT_MONTH").toString()+"月工考勤汇总数据信息", "UTF-8");
	...
	private List<LinkedHashMap> getDatasource(List<String> codeList, Object obj) {
		return attMonthReportDao.getDatasource(codeList, obj);
	}
	...
	public int insertComputer(List<Map> list) {
		return attMonthReportDao.insertComputer(list);
