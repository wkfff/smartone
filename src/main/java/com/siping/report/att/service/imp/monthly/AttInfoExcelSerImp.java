package com.siping.report.att.service.imp.monthly;
...
@Service
public class AttInfoExcelSerImp implements AttInfoExcelSer {

	Logger logger = Logger.getLogger(AttInfoExcelSerImp.class);

	@Autowired
	private AttInfoReportDao attInfoReportDao;
	...
	public void viewAttInfoReport(HttpServletRequest request,
		...
		// 页面提交的JSON信息
		String jsonCodeString = attInfoReportDao.getAttInfoColumnInfo(paramMap); 
		...
		List<String> titleList = this.attInfoReportDao
				.getAttInfoTitleInfo(paramMap);

		// 1.创建一个 workbook
		HSSFWorkbook workbook = new HSSFWorkbook();

		// 2.创建一个 worksheet
		HSSFSheet worksheet = workbook.createSheet(paramMap.get("ATT_MONTH").toString());
		...
		Layouter.buildAttMonthReport(worksheet, startRowIndex, startColIndex, titleList);
		Layouter.buildAttInfoReport(worksheet, startRowIndex, startColIndex, titleList);
		...
		try {
			fileName = java.net.URLEncoder.encode(paramMap.get("ATT_MONTH").toString()+"月工考勤数据信息", "UTF-8");
	...
	private List<LinkedHashMap> getDatasource(List<String> codeList, Object obj) {
		return attInfoReportDao.getDatasource(codeList, obj);
	}
	...
	public int insertComputer(List<Map> list) {
		return attInfoReportDao.insertComputer(list);
